...
// ----------------------------------------------------------------------
// Parse l'énergie à partir du nom du fichier
//   output_mono_n_500keV_Tetra_summary.txt -> 0.5 MeV
//   output_mono_n_1_Tetra_summary.txt      -> 1 MeV
// ----------------------------------------------------------------------
static double parseEnergyFromName(const std::string &name) {
  std::regex rg("output_mono_n_([^_]+)_Tetra_summary\\.txt");
  std::smatch m;
  if (!std::regex_search(name, m, rg)) return -1.0;

  std::string tok = m[1]; // ex: "500keV" ou "1"
  std::smatch mk;
...
//   s’adapte au format actuel :
//   - "Total events: N"
//   - "Events (all detections <= gate): N (eff = X +/- dX %)"
//   - "Mean measured multiplicity in gate: Y"
// ----------------------------------------------------------------------
static Point parseSummaryFile(const std::string &path) {
  ...
  // Mean measured multiplicity (ancien ou nouveau label)
  ...
  //  -> anciens fichiers (pas de +/-, par sécurité)
  ...
  if (err_from_line >= 0.0) {
    // si elle est écrite dans le summary, on la prend
    p.eff_unc_percent = err_from_line;
  } else {
    // sinon, on recalcule une erreur statistique
    if (p.meanMult >= 0.0 && p.totalEvents > 0) {
      // approx. : sigma(eff) ≈ 100 * sqrt(<mult> / N) / ?
      // Ici tu avais : 100*sqrt(meanMult/N). On garde la même formule.
                     ...
                     const char* outprefix = "./mono_eff_scan")
{
  ...
  while ((ent = gSystem->GetDirEntry(dp))) {
    std::string s(ent);
    if (s.find("output_mono_n_") == 0 &&
        s.find("_Tetra_summary.txt") != std::string::npos) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    g->SetPointError(i, 0.0, pts[i].eff_unc_percent);
  }
  ...
  leg->AddEntry(g,"Mean measured multiplicity (x100%)","p");
