...
//   - hPrior (optionnel) : TH1 défini sur l'axe TRUE (nbinsY), sert de prior f^(0)
...
TH1D* IterativeUnfoldBayes(const TH1*  hMeas,
                           const TH2D* hResp,
                           double ngenPerEtrue = 1e7,     // <-- NEW (default keeps old behavior)
                           int   maxIter     = 100,
                           ...
                           const TH1*  hPrior = nullptr)
{
  ...
  if (ngenPerEtrue <= 0) {
    std::cerr << "[ERROR] IterativeUnfoldBayes: ngenPerEtrue must be > 0." << std::endl;
    return nullptr;
  }
  ...
  // Si prior fourni : il doit être sur l'axe TRUE
  if (hPrior) {
    if (hPrior->GetNbinsX() != nbinsY) {
      std::cerr << "[ERROR] IterativeUnfoldBayes: hPrior nbinsX ("
                << hPrior->GetNbinsX() << ") != hResp Y-axis bins ("
                << nbinsY << "). Prior must be on TRUE axis." << std::endl;
      return nullptr;
    }
  }
  ...
  for (int ix = 1; ix <= nbinsX; ++ix) {
    for (int iy = 1; iy <= nbinsY; ++iy) {
      double v = hR2D->GetBinContent(ix, iy) / ngenPerEtrue;   // <-- CHANGED (was /1e07)
      if (v < 0) v = 0.0;
      R[ix-1][iy-1] = v;
    }
  }

  // efficacité ε_j = sum_i R_ij
  ...
  if (hPrior) {
    double sumPrior = 0.0;
    for (int j = 0; j < nbinsY; ++j) {
      double v = hPrior->GetBinContent(j+1);
      if (v < 0) v = 0.0;
      f[j] = v;
      sumPrior += v;
    }
    if (sumPrior > 0.0 && sumG > 0.0) {
      const double factor = sumG / sumPrior;
      for (int j = 0; j < nbinsY; ++j) f[j] *= factor;
    }
    if (verbose) {
      std::cout << "[Bayes] Using user prior on TRUE axis (renorm to sumG=" << sumG << ")\n";
    }
  } else {
  ...
  for (int k = 0; k < maxIter; ++k) {
    ...
    for (int j = 0; j < nbinsY; ++j) {
      if (eff[j] <= 0) { f_new[j] = 0.0; continue; }

      double sumOverMeas = 0.0;
      for (int i = 0; i < nbinsX; ++i) {
        if (D[i] <= 0) continue;
        double w_ij = R[i][j] * f_prev[j] / D[i];
    ...
    if (enforcePos) {
      for (int j = 0; j < nbinsY; ++j) if (f_new[j] < 0) f_new[j] = 0.0;
    }
    ...
    if (prevChi2NDF > 0 && tolRelChi2 > 0) {
      double relDiff = std::fabs(chi2NDF - prevChi2NDF) / ((chi2NDF + prevChi2NDF)/2.0);
  ...
  for (int j = 0; j < nbinsY; ++j) hUnfold->SetBinContent(j+1, f[j]);

  if (verbose) std::cout << "[Bayes] Done. Iterations performed: " << nIterDone << std::endl;
